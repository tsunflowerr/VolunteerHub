...
export async function searchEvents(req, res) {
    try {
        ...
        const cacheKey = `search:events:${JSON.stringify({ keyword, category, location, startDate, endDate, status, sort, page, limit })}`;
        ...
        if (type && type !== 'all') {
            ...
            if (type === 'posts') {
                const [posts, total] = await Promise.all([
                    ...
                    .select('title content author eventId likesCount commentsCount createdAt')
