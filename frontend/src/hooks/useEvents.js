...
export const useEventNames = () => {
  return useQuery({
    ...
    select: (data) => data.data.map(event => ({ id: event._id, name: event.name })),
...
export const useUpdateEvent = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      // Invalidate list and the specific event
      queryClient.invalidateQueries(eventKeys.lists());
      queryClient.invalidateQueries(eventKeys.detail(variables.id));
